...
class StripeWH_Handler:
    ...
    def _send_confirmation_email(self, order):
        ...
        # render_to_string will render our  confirmation_email files
        # as a string n allow us pass context just like we would pass
        # to a template.
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        # we strt wit profile=none, so we cn let anonymous users chkout
        # see notes
        profile = None
        username = intent.metadata.username
        if username != 'AnonymousUser':
            profile = UserProfile.objects.get(user__username=username)
            # If user clicked saveinfo we update their profile with shipping
            # details as their default delivery info
            if save_info:
        ...
        while attempt <= 5:
            try:
                ...
                time.sleep(1)
        if order_exists:
            # if d order exist in db, send confrimtn email
            # b4 sendn a response to stripe
            ...
            try:
                ...
                return HttpResponse(
                    ...
                    status=500)
        # if oder was created by webhook handler
        # send confrimtn email
        self._send_confirmation_email(order)
        return HttpResponse(
            content=f'Webhook received: {event["type"]} |\
